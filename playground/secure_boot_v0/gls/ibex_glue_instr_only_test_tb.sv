// ibex_glue_instr_only_test_tb.sv -- new file.
//
// Bisection stage 1.8: runs ibex_glue_nomem_boot.S (bare `lui s0,0x2` +
// infinite loop) through ibex_glue_instr_only_test.nl.v (ibex_top + a
// SINGLE ibex_to_tlul_host instance for instr fetch + real IMEM macro;
// data side has no glue module at all, just a trivial always-ready stub).
// See ibex_glue_instr_only_test.sv's header for what this isolates.
//
// PASS/FAIL: backdoor-probe s0 (x8) directly from the register file.
`timescale 1ns/1ps

module ibex_glue_instr_only_test_tb;
  logic clk;
  logic rst_n;
  ...
  initial begin
    ...
    #1;
    $display("");
    $display("[IBEX_GLUE_INSTR_ONLY] t=%0t final s0 = 0x%08h (expect 0x00002000)", $time, s0_reg);
    $display("[IBEX_GLUE_INSTR_ONLY] total s0 changes=%0d", s0_change_count);
    $display("[DIAG2] total u_imem.tl_i changes=%0d", imem_tl_i_changes);
    pass = (s0_reg == 32'h00002000);
    if (pass) $display("[IBEX_GLUE_INSTR_ONLY] PASS");
    else      $display("[IBEX_GLUE_INSTR_ONLY] FAIL");
    $finish;
  end
