# ibex_top — block-level timing constraints (gf180mcuD, OpenSTA-compatible).
# Single synchronous core clock (clk_i). On-chip block: I/O delays model neighbour
# register stages (the SoC fabric / memories), not off-chip delay.
...
# On-chip I/O budget: ~20% of the period reserved per boundary for neighbour logic.
