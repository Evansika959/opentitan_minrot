// 10-bank (10 KiB) variant of tlul_sram_if_macro_ocd_7bank -- byte-for-byte
// the same module, just NumBanks bumped and renamed for its own standalone
// PD hardening run (pd/blocks/tlul_sram_if_macro_ocd_10bank). The
// underlying logic (bank_sel one-hot, gen_macros generate loop) was
// already fully NumBanks-parameterized in the 7-bank file -- no logic
// changes needed here, only the module name (each hardened macro gets its
// own named instance by convention) and the default NumBanks.
module tlul_sram_if_macro_ocd_10bank #(
  ...
  parameter int unsigned NumBanks  = 10          // number of 1 KiB (256-word) tiles
) (
  input  logic                  clk_i,
  input  logic                  rst_ni,
...
endmodule : tlul_sram_if_macro_ocd_10bank
