// 4-bank (4 KiB) variant of tlul_sram_if_macro_ocd_2bank -- byte-for-byte
// the same module, just NumBanks bumped and renamed for its own standalone
// PD hardening run (pd/blocks/tlul_sram_if_macro_ocd_4bank). Fully
// NumBanks-parameterized already (bank_sel one-hot, gen_macros generate
// loop) -- no logic changes needed, only the module name and default.
...
module tlul_sram_if_macro_ocd_4bank #(
  ...
  parameter int unsigned NumBanks  = 4           // number of 1 KiB (256-word) tiles
) (
  input  logic                  clk_i,
  input  logic                  rst_ni,
...
endmodule : tlul_sram_if_macro_ocd_4bank
