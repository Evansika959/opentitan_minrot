// top_pd_min.sv -- new file, derived from top_pd_7bank.sv.
//
// Full-chip PD integration top, right-sized to the REAL minimum memory
// footprints measured during the secure-boot + PUF bring-up session
// instead of top_pd_7bank's uniform-7KiB-everywhere choice:
//   - u_imem: tlul_sram_if_macro_ocd_10bank (10 KiB) -- the PUF-derived-
//     key ROM (compute_public_key + verify + copy + jump) measured
//     ~9648-9976 bytes, which overflows a 7KiB macro by ~35%.
//   - u_esram, u_dmem: tlul_sram_if_macro_ocd_2bank (2 KiB each) -- the
//     packed signed container measured 240 bytes and the demo payload
//     measured 133 bytes; a 7KiB macro for either was ~30-50x overkill.
// Both macro blocks (pd/blocks/tlul_sram_if_macro_ocd_10bank,
// pd/blocks/tlul_sram_if_macro_ocd_2bank) passed DRC/LVS/STA cleanly and
// are exported into pd/macros/ via export_macros.sh, same as the 7-bank
// block was.
//
// Architecture otherwise unchanged from top_pd_7bank.sv/top_pd_puf.sv
// (mixed flow): ibex core, uart, xbar_tlul_2to5, and the PUF TL-UL glue
// (tlul_adapter_reg + sram_puf_regif) are all synthesized directly from
// RTL as flat std-cell logic -- only the three memory macros and the
// PUF's own foundry entropy-source SRAM (gf180mcu_fd_ip_sram__
// sram64x8m8wm1) are hard macros/blackbox stubs, physical views coming
// from MACROS in config.json.
module top_pd_min (
  input  logic clk_i,
  input  logic rst_ni,
  ...
  // ================= Memories (hard macros, right-sized per block) =================
  tlul_sram_if_macro_ocd_10bank u_imem  (.clk_i, .rst_ni, .tl_i(tl_to_rom),       .tl_o(tl_from_rom),       .en_ifetch_i(prim_mubi_pkg::MuBi4True));
  tlul_sram_if_macro_ocd_2bank  u_esram (.clk_i, .rst_ni, .tl_i(tl_to_esram),     .tl_o(tl_from_esram),     .en_ifetch_i(prim_mubi_pkg::MuBi4True));
  tlul_sram_if_macro_ocd_2bank  u_dmem  (.clk_i, .rst_ni, .tl_i(tl_to_dmem_sram), .tl_o(tl_from_dmem_sram), .en_ifetch_i(prim_mubi_pkg::MuBi4False));
