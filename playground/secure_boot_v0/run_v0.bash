...
fusesoc --cores-root ../.. run --target=sim --tool=verilator \
        xinting:playground:secure_boot_v0 > sim.log 2>&1 \
...
if grep -q "UART_RX" sim.log; then
  grep "UART_RX" sim.log | sed 's/^/  /'
  STREAM="$(grep -oE "byte=0x[0-9a-f]+ \('.'\)" sim.log | sed -E "s/.*\('(.)'\)/\1/" | tr -d '\n')"
  echo
  printf '  decoded stream: \033[1;36m%s\033[0m\n' "$STREAM"
  echo
  echo "  Legend (each boot stage prints short progress markers):"
  echo "    ROM      s = start,  J = before payload copy,  P = after copy -> jump to ROM_EXT"
  echo "    ROM_EXT  EXT = entered,  J = before copy,  P = after copy -> jump to BL0"
  echo "    BL0      B = entered"
  echo "    => expected full chain: 'sJPEXTJPB'  (ROM -> ROM_EXT -> BL0)"
  echo
  echo "  Note: the loaded ROM (test_sw/c/rom.c) is a debug build with the on-target"
  echo "  ECDSA signature check commented out (for sim speed). The images are still"
  echo "  really signed/packed; see README to enable full verification."
else
  echo "  (no UART bytes decoded — check sim.log)"
fi
